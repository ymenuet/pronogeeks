...
import { updateMatchweekPronogeeks } from './helpers';
import { printError, copyReducer, hasMatchStarted } from '../../utils/helpers';
import {
...
export const getUserMatchweekPronos =
  (userID, seasonID, matchweekNumber) => async (dispatch, getState) => {
    ...
    try {
      const {
        data: { pronogeeks },
      } = await pronogeekService.get(
        `/geek/${userID}/season/${seasonID}/matchweek/${matchweekNumber}`
      );
      ...
      newPronogeeks[`${seasonID}-${matchweekNumber}`] = {
        ...
        error: printError(
          'fr',
          error,
          'Erreur lors du chargement de tes pronogeeks. Recharge la page ou réessaye plus tard.'
        ),
      };
      dispatch({
        type: ADD_USER_PRONOGEEKS,
  ...
  (geekID, seasonID, matchweekNumber) => async (dispatch, getState) => {
    ...
    try {
      const {
        ...
        `/geek/${geekID}/season/${seasonID}/matchweek/${matchweekNumber}`
      );
      ...
      newPronogeeks[`${geekID}-${seasonID}-${matchweekNumber}`] = {
        ...
        error: printError(
          'fr',
          error,
          'Erreur lors du chargement des pronogeeks. Recharge la page ou réessaye plus tard.'
        ),
      };
      dispatch({
        type: ADD_GEEK_MATCHWEEK_PRONOGEEKS,
    ...
    try {
      ...
      newPronogeeks[`${fixtureID}-${geekleagueID}`] = {
        ...
        error: printError(
          'fr',
          error,
          'Erreur lors du chargement des pronos de la ligue geek. Recharge la page ou réessaye plus tard.'
        ),
      };
...
export const handleInputHomeProno = (homePronoInputValue, fixture) => (dispatch, getState) => {
  ...
  newPronogeeks[`${season}-${matchweek}`][_id].modified =
    isAwayPronoSet && !isSamePronoAsInDB && homeProno >= 0;
...
export const handleInputAwayProno = (awayPronoInputValue, fixture) => (dispatch, getState) => {
  ...
  newPronogeeks[`${season}-${matchweek}`][_id].modified =
    isHomePronoSet && !isSamePronoAsInDB && awayProno >= 0;
...
export const saveAllPronogeeks =
  (seasonID, matchweekNumber, fixtures) => async (dispatch, getState) => {
    ...
    if (
      !Object.keys(newPronogeeks[`${seasonID}-${matchweekNumber}`]).filter(
        (key) => key !== 'error' && key !== 'saving' && key !== 'saved'
      ).length
    ) {
    ...
    const pronogeeksToSave = Object.values(newPronogeeks[`${seasonID}-${matchweekNumber}`])
      .filter(({ modified, fixture }) => modified === true && !fixturesStartedIDs.includes(fixture))
      .map(({ fixture, homeProno, awayProno }) => ({
        fixture,
        homeProno,
        awayProno,
      }));
...
export const resetMatchweekSaveAndErrorState =
  (seasonID, matchweekNumber) => (dispatch, getState) => {
    const newPronogeeks = copyReducer(getState, PRONOGEEK_REDUCER_KEY, USER_PRONOGEEKS_KEY);
