...
export default (event, setFileName = null) => {
  ...
  if (event.target.files.length > 0) {
    ...
    if (file.size > 1000000)
      openNotification('warning', 'Attention', 'La taille du fichier ne peux pas excéder 1Mo.');
    else if (file.type !== 'image/jpeg' && file.type !== 'image/png' && file.type !== 'image/jpg')
      openNotification('warning', 'Attention', 'La photo doit être au format JPG ou PNG.');
    else {
