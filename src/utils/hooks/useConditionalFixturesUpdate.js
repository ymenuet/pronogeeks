...
export const useConditionalFixturesUpdate = ({
  ...
  useEffect(() => {
    if (isUserConnected && !newSeason && fixtures) {
      ...
      const fixtureOddsUpdates = fixtures.map((fixture) =>
        new Date(fixture.lastOddsUpdate).getTime()
      );
      ...
      if (
        ...
        const matchweekNotFinished = !!fixtures.filter(
          ({ statusShort }) => !isMatchFinished(statusShort)
        ).length;
        const fixturesUpdatedMoreThanThirtyMinutesAgo =
          Date.now() > lastUpdate + MILLISECONDS_IN_30_MINUTES;

        if (fixturesUpdatedMoreThanThirtyMinutesAgo && matchweekNotFinished && !scoresUpdatedOnce) {
        ...
        if (!allFixturesStarted && oddsUpdatedMoreThanOneDayAgo && !oddsUpdatedOnce) {
