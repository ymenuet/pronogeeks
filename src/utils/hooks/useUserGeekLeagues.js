...
export const useUserGeekLeagues = () => {
  ...
  const geekleagues = useSelector(({ geekleagueReducer }) => geekleagueReducer.geekleagues);

  const dispatch = useDispatch();

  useEffect(() => {
    if (isUserConnected) {
      const geekleaguesNotLoadingNorWithError =
        !isEmpty(geekleagues) && !geekleagues.loading && !geekleagues.error && !geekleagues.empty;
